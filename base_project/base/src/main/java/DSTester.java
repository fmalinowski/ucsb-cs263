...
public class DSTester {
	public static void main(String[] args) {
		...
		Response response = service
				...
				.put(Entity.entity(val, MediaType.APPLICATION_XML),
						Response.class);

		// Return code should be 201 == created resource
		int status = response.getStatus();
		if (status != 201) {
			System.out.println("Test 1 Failed: Expected status 201, got: "
					+ status);
		} else {
		...
		if (status != 204) {
			System.out.println("Test 2 Failed: Expected status 204, got: "
					+ status);
		} else {
		...
		String s;
		try {
			s = service.path("rest").path("ds").request()
		...
		passed = true;
		try {
			s = service.path("rest").path("ds").path(keyname).request()
					.accept(MediaType.APPLICATION_XML).get(String.class);
			// s =
			// service.path("rest").path("ds").path(keyname).request(MediaType.APPLICATION_XML).get(String.class);
		} catch (Exception e) {
			passed = false;
		...
		if (!passed) {
			System.out.println("Test 7 Failed: " + keyname
					+ " found in datastore following delete.  DB dump:");
			// dumping DB
			System.out.println(service.path("rest").path("ds").request()
					.accept(MediaType.APPLICATION_XML).get(String.class));
		}
		...
		status = response.getStatus();
		if (status != 204) {
			System.out.println("Test 8 Failed: Expected status 204, got: "
					+ status);
		} else {
